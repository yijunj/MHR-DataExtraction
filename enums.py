...
def enum_MaterialCategory(material_category):
    # TODO: MHRice has no info, but there should be something...
...
def enum_QuestType(quest_type):
    quest_type_dict = {str(0x00000001): 'HUNTING',\
                        str(0x00000002): 'KILL',\
                        str(0x00000004): 'CAPTURE',\
                        str(0x00000008): 'BOSSRUSH',\
                        str(0x00000010): 'COLLECTS',\
                        str(0x00000020): 'TOUR',\
                        str(0x00000040): 'ARENA',\
                        str(0x00000080): 'SPECIAL',\
                        str(0x00000100): 'HYAKURYU',\
                        str(0x00000200): 'TRAINING'}
    if quest_type == 0:
        return 'empty'
    else:
        return quest_type_dict[str(quest_type)]
...
########################################
# These are listed in MHRice condition_damage_data.rs and condition_damage_preset.rs
def enum_StanceStatusFlags(key):
    stance_list = ['Stand', 'Fly', 'Diving', 'Wall', 'Ceiling']
    return [stance_list[4-i] for i in reversed(range(5)) if '{0:5b}'.format(key)[i] == '1']
...
def enum_DamageCategoryFlag(key):
    # TODO
    return key
    # stance_list = ['Stand', 'Fly', 'Diving', 'Wall', 'Ceiling']
    # return [stance_list[4-i] for i in reversed(range(5)) if '{0:5b}'.format(key)[i] == '1']
